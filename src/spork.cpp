...
std::vector<CSporkDef> sporkDefs = {
    MAKE_SPORK_DEF(SPORK_1_ANONRESTRICT_ENABLED,   4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_2_ANONSTANDARD_ENABLED,   4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_3_DANDELION_ENABLED,      4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_4_CHAINHALT_ENABLED,      4070908800ULL), // OFF
};
...
bool CSporkMessage::Sign(const CKey& key)
{
    ...
    if (!CHashSigner::VerifyHash(hash, pubKeyId, vchSig, strError)) {
        LogPrintf("CSporkMessage::Sign -- VerifyHash() failed (%s)\n", strError);
        return false;
    }
    return true;
}
...
bool CSporkMessage::CheckSignature(const CKeyID& pubKeyId) const
{
    ...
    std::string strError;
    if (!CHashSigner::VerifyHash(hash, pubKeyId, vchSig, strError)) {
        LogPrint(BCLog::SPORK, "CSporkMessage::CheckSignature -- VerifyHash() failed (%s)\n", strError);
        return false;
    }
    return true;
}
