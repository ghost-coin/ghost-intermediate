...
void ParseRecipients(const UniValue& address_amounts, const UniValue& subtract_fee_outputs, std::vector<CRecipient> &recipients) {
    ...
    for (const std::string& address: address_amounts.getKeys()) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Ghost address: ") + address);
        }
        ...
        HELP_REQUIRING_PASSPHRASE,
            ...
            "       \"" + FeeModes("\"\n\"") + "\""},
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 GHOST with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 GHOST with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 GHOST with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 GHOST with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
...
static RPCHelpMan lockunspent()
{
    return RPCHelpMan{"lockunspent",
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending ghost.\n"
...
static RPCHelpMan createwallet()
{
    return RPCHelpMan{
        ...
        "\nCreates and loads a new wallet.\n",
        {
            ...
            {"use_legacy", RPCArg::Type::BOOL, /* default */ "false", "Use legacy bip44 prefix when creating wallet."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fUseLegacy = !request.params[5].isNull() && request.params[5].get_bool();
    ...
    bilingual_str error;
    Optional<bool> load_on_start = nullopt;
    std::shared_ptr<CWallet> wallet = CreateWallet(*context.chain, request.params[0].get_str(), load_on_start, options, status, error, warnings, fUseLegacy);
    if (!wallet) {
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress") || options.exists("change_address")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid ghost address");
            }
...
RPCHelpMan getaddressinfo()
{
    return RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given ghost address.\n"
...
static RPCHelpMan send()
{
    return RPCHelpMan{"send",
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    ...
                    {"change_address", RPCArg::Type::STR_HEX, /* default */ "pool address", "The ghost address to receive the change"},
        ...
        RPCExamples{""
        "\nSend 0.1 GHOST with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 GHOST with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 GHOST with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 GHOST with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "createwallet",                     &createwallet,                  {"wallet_name", "disable_private_keys", "blank", "passphrase", "avoid_reuse", "use_legacy"} },
